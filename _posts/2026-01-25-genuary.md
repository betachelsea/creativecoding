...
class Arrow
    ...
    def move(dt, goal_x, goal_y)
        ...
        theta = th_gc - th_pc
        rotate_v = @rotation_speed * dt # 回転速度->ωt
        rotate_v = @rotation_speed * (millis() / 1000.0)
        text("rotate_v:#{rotate_v}", 20, 80) # うーん?
        # @th = @th - rotate
        ...
        # 楕円運動
        @cx = goal_x + @a * Math.cos(rotate_v)
        @cy = goal_y + @b * Math.sin(rotate_v)
    end
